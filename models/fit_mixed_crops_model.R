...
null_model <- glmer(disagree ~ 1 + (1|crop_id), 
...
sink('tmp/mecrops_model_summary.txt')
print(summary(model))
sink()
...
write.csv(fixed, 'tmp/mecrops_fixed_effects.csv')
...
write.csv(random, 'tmp/mecrops_random_effects.csv', row.names=FALSE)

# Save predictions
predictions <- predict(model, type='response')
write.csv(data.frame(pred=predictions), 'tmp/mecrops_predictions.csv', row.names=FALSE)
...
write.csv(fit_stats, 'tmp/mecrops_fit_stats.csv', row.names=FALSE) 
...
null_model <- glmer(disagree ~ 1 + (1|user_id), 
...
sink('tmp/meworkers_model_summary.txt')
print(summary(model))
sink()
...
write.csv(fixed, 'tmp/meworkers_fixed_effects.csv')
...
write.csv(random, 'tmp/meworkers_random_effects.csv', row.names=FALSE)

# Save predictions
predictions <- predict(model, type='response')
write.csv(data.frame(pred=predictions), 'tmp/meworkers_predictions.csv', row.names=FALSE)
...
write.csv(fit_stats, 'tmp/meworkers_fit_stats.csv', row.names=FALSE) 
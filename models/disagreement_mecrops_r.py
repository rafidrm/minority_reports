...
def fit_disagreement_mecrops_model(df, model_name='disagreement_mecrops_r', dataset='ECPD'):
    """
    Fit mixed-effects logistic regression using R's lme4 with crop random effects
    """
    # Get feature columns
    ...
    print(f"\nFitting mixed-effects model with random crop effects...")
    ...
    # Clean column names for R compatibility and ensure we include crop_id
    required_cols = ['disagree', 'crop_id'] + feature_cols
    ...
    model_df.to_csv('tmp/mecrops_model_data.csv', index=False)
    
    # Run R script with arguments
    try:
        print("\nFitting models in R...")
        r_script_path = Path(__file__).parent / 'fit_mixed_crops_model.R'
        subprocess.run([
            'Rscript',
            str(r_script_path),
            'tmp/mecrops_model_data.csv',
            ','.join(feature_cols)
        ], check=True)
        
        # Read results with updated paths
        fit_stats = pd.read_csv('tmp/mecrops_fit_stats.csv')
        crop_effects = pd.read_csv('tmp/mecrops_random_effects.csv')
        predictions = pd.read_csv('tmp/mecrops_predictions.csv')  # Read predictions early
        ...
        class Results:
            def __init__(self):
                ...
                self.random_effects = crop_effects
        ...
        tmp_files = [
            'mecrops_model_data.csv',
            'mecrops_fit_stats.csv',
            'mecrops_fixed_effects.csv',
            'mecrops_random_effects.csv',
            'mecrops_predictions.csv',
            'mecrops_model_summary.txt'
        ]
        for f in tmp_files:
            try:
                os.remove(f'tmp/{f}')
...
def save_model_results(results, model_name, dataset='ECPD'):
    ...
    save_model_files(model_name, {
        'mecrops_model_summary.txt': 'summary.txt',
        'mecrops_predictions.csv': 'predictions.csv',
        'mecrops_fixed_effects.csv': 'fixed_effects.csv',
        'mecrops_random_effects.csv': 'random_effects.csv'
    }, dataset=dataset)
    
    # Update paths for plots
